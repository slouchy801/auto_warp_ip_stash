...
// 直接寫死版本，唔再讀取檔案，防止 Vercel 崩潰
const currentVersion = "v2.1.6";
        ...
        try {
            ...
            .then(data => {
                if (data && data.result) {
                    ...
                    if (typeof result === 'string' && result.startsWith('%7B')) {
...
export default async function handler(request, response) {
    try {
            ...
            <div class="container">
                ...
                <div class="card yaml-container">
                    ...
                    <label for="yaml-toggle" class="yaml-header">
                        <h2>📄 當前純淨 YAML 預覽（經典 8 節點還原版） <span class="arrow-icon">▼</span></h2>
                    </label>
        ...
        // 最外層極致防禦，即使爆錯都唔會令 Vercel 出 500 畫面
        response.setHeader('Content-Type', 'text/plain; charset=utf-8');
        return response.status(500).send("Server Error: " + globalError.message);
    }
}            hostname: u.hostname, path: u.pathname, method: 'POST',
            headers: { 
                'Content-Type': 'application/json', 
